...
local function decode(buf, sym)
  ...
  if has_table then
    ...
    for i=1,count do
      ...
      local count = d:GetU30()

      for i=1,count do
        local vi = d:GetU30()
        local v = sym_map_vk[vi] or index_map[vi]

        if not ref_count[vi] then
          ref_count[vi] = 1
        else
          ref_count[vi] = ref_count[vi] + 1
        end
      ...
      while d:available() > 0 do
        ...
        if not ref_count[ki] then
          ref_count[ki] = 1
        else
          ref_count[ki] = ref_count[ki] + 1
        end
        if not ref_count[vi] then
          ref_count[vi] = 1
        else
  ...
  for i,v in ipairs(list) do
    if i < 127 then
      table.insert(t, sym_map_vk[v[1]] or index_map[v[1]])
    end
  end
...
local function encode(obj, sym)
  ...
  if (#ctx.tables) > 0 then
    ...
    for i,tb in ipairs(ctx.tables) do
      ...
      for k,v in pairs(tb) do
        if type(k) ~= "number" or k > tb_count or k <= 0 then
      ...
      bodystream:AddU30(d2:available() + d:available())
      bodystream:AddBytes(d2:GetBytes())
      if d:available() > 0 then
        bodystream:AddBytes(d:GetBytes())
      end
    end
  end
